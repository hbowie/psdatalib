...
public class TextIOxml 
  ...
  public void startElement (
      ...
      nextNode.setType (localName);
      currentNode.add(nextNode);
      currentNode = nextNode;
      for (int i = 0; i < attributes.getLength(); i++) {
        TextNode attrNode = new TextNode(tree);
        attrNode.setType(attributes.getLocalName (i));
        attrNode.setAttribute(true);
        attrNode.setText(attributes.getValue(i));
        currentNode.add(attrNode);
      } 
    ...
    if (allWhitespace) {
      ...
      if (currentNode != null) {
        if (currentNode.hasChildTags()) {
          ...
          currentNode.add (nextNode);
          // currentNode = nextNode;
          nextNode.characters (ch, start, length);
        } else {
  ...
  public void endElement (
    ...
    if (currentNode != null) {
      ...
      if (currentNode.getType().equalsIgnoreCase(localName)) {
        TextNode parentNode = (TextNode)currentNode.getParent();
        if (parentNode != null) {
