...
/**
   ...
   Version History: <ul><li>
    2003/07/25 - Fixed a bug, introduced in the last change, that returned
                 the wrong (next) file name in each data record returned. 
    2003/05/31 - Added the nextFileIn method to return directory
                 entries as File objects rather than DataRecords. <li>
    2003/01/27 - Corrected a bug that was returning bad path information
                 in the new Folder columns. <ii>
    2002/12/02 - Broke last modified date and time into two separate fields.
                 The date field is in the form "yyyy/MM/dd" so that it can be
                 easily sorted. The time field is in the form "hh:mm:ss tmz" 
                 with a military (24-hour) time, again for easy sorting. <br>
                 Added explosion depth so that sub-directories can be read
                 as well as a top-level directory. <br>
                 Added folder (sub-directory) fields to output returned. <li>
    2002/11/19 - Added openForInput method with Record Definition for 
                 consistency with DataSource interface. <li>
