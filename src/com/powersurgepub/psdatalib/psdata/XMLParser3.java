...
public class XMLParser3 
  ...
  private               boolean               xmlEntityDecoding = false;
  private               StringConverter       xmlEntityDecoder = null;
   ...
   */
  public XMLParser3 (String inFileName) {
    super (inFileName);
    commonConstruction();
  }
  ...
  public void setXmlEntityDecoding (boolean xmlEntityDecoding) {
    this.xmlEntityDecoding = xmlEntityDecoding;
    if (xmlEntityDecoding) {
      xmlEntityDecoder = new StringConverter();
      xmlEntityDecoder.addXML();
      xmlEntityDecoder.setForward(false);
      xmlEntityDecoder.prepare();
    }
  }
  
  public boolean getXMLEntityDecoding () {
    return xmlEntityDecoding;
  }
  ...
  public DataRecord nextRecordIn () 
    ...
    do {
      ...
      if (wordType == NAME_TYPE) {
        ...
        if (nameLevel == 0) {
          docTag = word.toString();
        }
        else
        if (nameLevel == 1) {
          recTag = word.toString();
        }
        else
        if (nameLevel == 2) {
          recDef.addColumn(word.toString());
        }
        nameLevel++;
      } 
      else
      ...
      if (wordType == DATA_TYPE) {
        if (nameLevel >= 3) {
          String data;
          if (xmlEntityDecoding) {
            data = xmlEntityDecoder.convert (word.toString());
          } else {
            data = xmlEntityDecoder.convert(word.toString());
          }
          DataField dataField 
              = new DataField (recDef, recDef.getNumberOfFields() - 1, data.trim());
          dataRec.addField(dataField);
        }
      }
    } while ((! isAtEnd())
        && (nameLevel > 1
  ...
  int storeName (int level, String value) {
    ...
    column = dataRec.storeField (recDef, fieldName, value.trim());
    return column;
  }
  ...
  int storeField (String fieldName, String value) {
    ...
    if (nullDataTag == 3) {
      wordType = 3;
      word = new StringBuffer (saveName);
      nullDataTag = 0;
    } else {
